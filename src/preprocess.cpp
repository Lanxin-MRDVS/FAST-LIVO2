...
void Preprocess::s10u_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  pl_surf.clear();
  pcl::PointCloud<lx_ros::LxPointXYZIT> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  int plsize = pl_orig.size();
  pl_surf.reserve(plsize);

  double time_head = msg->header.stamp.toSec();
  std::cout << "time_head:" << long(time_head * 1000) << std::endl;
  for (int i = 0; i < plsize; ++i) {
    if (i % point_filter_num != 0)
      continue;
    ...
    added_pt.z = pt.z;
    added_pt.intensity = pt.intensity;
    added_pt.curvature = (pt.timestamp * 1e-6 - time_head) * 1e3; // ms
    pl_surf.points.push_back(added_pt);
  }
  std::sort(pl_surf.points.begin(), pl_surf.points.end(),
            [](const PointType &a, const PointType &b) {
