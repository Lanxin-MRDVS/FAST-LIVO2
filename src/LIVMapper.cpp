...
void LIVMapper::readParameters(ros::NodeHandle &nh) {
  ...
  nh.param<double>("time_offset/lid_time_interval", lid_time_interval, 0.0);
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas) {
  ...
  case LIVO: {
    ...
    case VIO: {
      ...
      ROS_INFO_STREAM(
          "[SYNC] last_lio_update_time:"
          << long(meas.last_lio_update_time * 1000)
          << ", lid_interval_time:" << long(lidar_time_interval * 1000)
          << ", lid_newest_time:" << long(lid_newest_time * 1000)
          << ", imu_newest_time:" << long(imu_newest_time * 1000)
          << ", img_capture_time:" << long(img_capture_time * 1000));
